...
export class BaileysStartupService extends ChannelStartupService {
  ...
  private async sendMessageWithTyping<T = proto.IMessage>(number: string, message: T, options?: Options) {
    ...
    if (number?.includes('@lid')) {
      // Lid chats are already valid WhatsApp conversation IDs, but the public
      // onWhatsApp() validation path rejects them. Send directly to the lid JID.
      sender = number.toLowerCase();
    } else {
